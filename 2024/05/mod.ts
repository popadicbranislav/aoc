...
function calculateMiddleValueSum(lists: string[][]): number {
  return lists.map(u => +u[Math.floor(u.length / 2)]).reduce((a, b) => a + b, 0)
}
  ...
  updates.forEach((update) => {
    ...
    for (let i = 0; i < update.length; i++) {
      ...
      rulesApplied.forEach((rule) => {
        if ((rule[0] === page && update.includes(rule[1]) && !update.slice(i + 1).includes(rule[1]))
          || (rule[1] === page && update.includes(rule[0]) && !update.slice(0, i).includes(rule[0])))
          pageCorrect = false
      });
