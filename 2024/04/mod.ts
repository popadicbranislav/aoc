...
function searchWord(rows: string[], dir: string, targetWord: string, currentCharIndex: number, { x, y }: Position): number {
  let wordsFound = 0
  ...
  if (targetWord.length - 1 === currentCharIndex && charToCheck === charToFind)
    return 1
  ...
  wordsFound += searchWord(rows, dir, targetWord, currentCharIndex + 1, nextPosition)
...
export function part2(data: string[]) {
  ...
  for (let y = 0; y < data.length; y++) {
    ...
    for (let x = 0; x < line.length; x++) {
      ...
      const posNE = { x: x + movementVectors['NE'].x, y: y + movementVectors['NE'].y }
      const posSW = { x: x + movementVectors['SW'].x, y: y + movementVectors['SW'].y }
      ...
      const posNW = { x: x + movementVectors['NW'].x, y: y + movementVectors['NW'].y }
      const posSE = { x: x + movementVectors['SE'].x, y: y + movementVectors['SE'].y }
